...
['GRID_IDAP_UPDATE',['Другорядне завдання','Обладнання IDAP']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
for '_x' from 0 to 1 step 0 do {
	...
	if ((_objectivesCompleted) || {(_serverTime > _missionTimeout)}) exitWith {
		...
		if (_serverTime > _missionTimeout) then {
			_text = 'Завдання прострочено';
			['GRID_IDAP_UPDATE',['Другорядне завдання завершено',_text]] remoteExec ['QS_fnc_showNotification',-2,FALSE];
		} else {
			{
				_objective = _x;
				_objective params [
