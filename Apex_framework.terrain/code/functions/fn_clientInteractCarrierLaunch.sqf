...
if (!(_cameraOn getVariable ['QS_carrier_launch',FALSE])) then {
	...
	_cameraOn spawn {
		...
		if (diag_tickTime > (uiNamespace getVariable ['QS_fighterPilot_lastMsg',(diag_tickTime - 1)])) then {
			...
			['sideChat',[WEST,'AirBase'],(format ['Близбка Повiтрянна Пiдтримка (%1) доступна. Пiлот: %2',(getText (configFile >> 'CfgVehicles' >> (typeOf _this) >> 'displayName')),profileName])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		};
